...
type ProjectStatus = { build?: { status?: string }; release?: unknown; deployment?: { url?: string; status?: string } };
...
export default function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const action = (name: string) => {
    ...
    fetch(`/api/appfactory/v1/projects/${id}/${name}`, { method: "POST" })
      ...
      .catch((reason: unknown) => setError(reason instanceof Error ? reason.message : `${name} 失败`))
  ...
  const send = () => {
    ...
    setBusy(true);
    setError("");
    fetch(`/api/appfactory/v1/sessions/${session.id}/run`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ prompt }) })
      ...
      .catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Pi 执行失败"))
      .finally(() => { setBusy(false); void refreshStatus(); });
  };
  ...
  return <main className="grid min-h-[calc(100vh-56px)] grid-cols-1 gap-4 p-3 sm:p-4 lg:grid-cols-[minmax(0,1fr)_320px]">
    ...
    <aside className="space-y-4"><div className="rounded-2xl border border-slate-200 bg-white p-4"><h2 className="text-sm font-semibold">文件与 Diff</h2><p className="mt-3 text-xs text-slate-400">Workspace 文件与最近改动将在这里显示</p></div><div className="rounded-2xl border border-slate-200 bg-white p-4"><h2 className="text-sm font-semibold">Preview / Build</h2><div className="mt-3 flex flex-wrap gap-2"><button disabled={busy} onClick={() => action("check")} className="rounded-lg border px-2 py-1 text-xs disabled:opacity-50">Check</button><button disabled={busy} onClick={() => action("build")} className="rounded-lg border px-2 py-1 text-xs disabled:opacity-50">Build</button><button disabled={busy} onClick={() => action("preview")} className="rounded-lg bg-indigo-600 px-2 py-1 text-xs text-white disabled:opacity-50">Preview</button><button disabled={busy} onClick={() => action("deploy")} className="rounded-lg bg-emerald-600 px-2 py-1 text-xs text-white disabled:opacity-50">Deploy</button></div><p className="mt-3 text-[11px] text-slate-500">Build：{status.build?.status ?? "未执行"} · Release：{status.release ? "已生成" : "未生成"} · Deployment：{status.deployment?.status ?? "未运行"}</p>{status.deployment?.url && <a href={status.deployment.url} target="_blank" rel="noreferrer" className="mt-2 inline-block text-xs text-indigo-600 hover:underline">打开本地应用 ↗</a>}</div></aside>
  </main>;
}

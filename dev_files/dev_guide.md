...
- `workflows`：列表页复用平台卡片页外框，内置 10 条演示流程，支持 `全部 / 业务审批 / 巡检运维 / 文档知识 / 客户服务` 标签筛选、搜索、新建、删除和流程资产卡片；画布页支持可视化拖拽编排 DAG 无环图，新建时初始仅生成“开始”节点并精简连线；提供 9 种节点类型（开始、结束、大模型、通用工具、业务 API、技能、代码块、条件、知识库）以及实体高亮线框顶栏组件；支持右侧抽屉双向更新参数，并完全由前端 `localStorage` 缓存持久化。

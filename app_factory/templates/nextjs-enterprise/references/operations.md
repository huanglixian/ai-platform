...
每个新项目会在创建 Workspace 时分配稳定的 Schema 名，并写入受管 Framework 与 `app.yaml`；这一步不连接数据库。首次发布与以后每次发布都会在该 Schema 上运行受管和业务迁移：创建 Schema、加 advisory lock、校验 checksum、只追加新迁移。Web 在迁移成功后才启动；有 `workerEntry` 时才额外启动应用自己的 Worker。

...
const statusMap = {
  运行中: "border-emerald-200 bg-emerald-50 text-emerald-700",
  待排期: "border-amber-200 bg-amber-50 text-amber-700",
      ...
      <div className="app-card overflow-hidden">
        <div className="flex min-h-[72px] items-center gap-3 border-b border-[#e7eef6] bg-[linear-gradient(180deg,#f2f7fd_0%,#eef4fb_100%)] px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-2">
              ...
              <div
                className={`shrink-0 rounded-[999px] border px-2.5 py-1 text-[10px] font-semibold shadow-none ${statusMap[workflow.status]}`}
              >
                {workflow.status}
              </div>
            </div>
        ...
        <div className="grid min-h-[60px] grid-cols-4 bg-white">
          {workflow.metrics.map((metric) => (
            ...
            >
              <div className="truncate leading-none text-title text-[15px] font-semibold">
                {metric.value}
              </div>

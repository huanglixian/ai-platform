        ...
        <Dialog.Popup className="fixed inset-y-0 right-0 z-50 flex h-full w-full max-w-md flex-col border-l border-border bg-white shadow-2xl focus:outline-none sm:w-[440px]">
          ...
          <footer className="border-t border-border px-5 py-4">
            ...
            {isExternal ? <div className="mt-2 grid grid-cols-2 gap-2">
              ...
              {app.isRemovable ? <button type="button" onClick={() => onRemove(app)} disabled={isPending} className="col-span-2 inline-flex h-9 items-center justify-center gap-2 text-xs font-medium text-rose-600 transition-colors hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60"><Trash2 size={14} />删除应用</button> : null}
            </div> : null}
          </footer>
        </Dialog.Popup>
      </Dialog.Portal>

...
项目 Workspace、Pi 会话与发布任务存放在 `data/storage/appfactory`；Release 会在 Worker 重启时按持久化部署记录恢复。应用中心的 AppFactory 卡片由发布流程管理，不能通过手工发布表单创建。`data/builtin` 是随 Git 管理的内置资源，`data/storage` 是本机运行数据，不应提交。

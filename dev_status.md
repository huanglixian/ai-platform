...
- Pi 发送态已支持即时清空输入、运行计时、停止/失败反馈；真实请求已通过 SSE 增量收到文本、完成事件，并按 `runId/sequence` 持久化。Pi JSONL 工具事件会展示为文件读取/修改、搜索、命令和上下文步骤，同一活动跨文本增量归并，AI 回复通过统一 Markdown/GFM 组件渲染。

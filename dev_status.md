...
- Pi 发送态已支持即时清空输入、运行计时、停止/失败反馈；真实请求已通过 SSE 增量收到文本、完成事件，并按 `runId/sequence` 持久化。Pi JSONL 工具事件会展示为文件读取/修改、搜索、命令和上下文步骤，同一运行收束在单一运行卡片内，步骤在卡片内部固定区域更新，完成/失败历史可展开查看；无工具步骤的成功问答不重复显示执行摘要；旧版 Pi 的启动诊断、内部完成标记和孤立遗留结果会在聊天层过滤；AI 回复通过统一 Markdown/GFM 组件渲染。

...
- 预览与发布：`app_factory/server/runtime-workspace.ts` 为 Preview 和 Release 构建准备隔离运行副本；`preview.ts` 只在 Preview 副本中以 Webpack 开发模式启动 Next，`release-builder.ts` 只在临时构建副本中执行 lint、typecheck 和 build。Pi 可以继续在真实 Workspace 使用 Bash；Pi 运行期间不能 Preview/发布，发布排队或运行期间不能开始 Pi，避免读写同一份源码。

...
export function ApplicationForm({ app, source, onClose, onSaved }: ApplicationFormProps) {
  ...
  const applicationSource = (app?.source ?? source) as Exclude<PlatformSource, "appfactory">;
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(isEditing ? `/api/agenthub/v1/applications/${app?.id}` : "/api/agenthub/v1/applications", {
        ...
        body: JSON.stringify(isEditing ? { name, description, entryUrl: url, launchCommand } : {
          name,
          description,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-sm" role="presentation">
      <section role="dialog" aria-modal="true" aria-labelledby="external-app-form-title" className="max-h-[calc(100vh-32px)] w-full max-w-xl overflow-y-auto rounded-xl border border-border bg-white p-5 shadow-2xl sm:p-6">
        <div className="flex items-start justify-between gap-4"><div><h2 id="external-app-form-title" className="text-base font-bold text-title">{isEditing ? `编辑${sourceLabel}应用` : `接入${sourceLabel}应用`}</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">{requiresLaunchCommand ? "保存名称、说明、启动命令和访问地址；应用代码无需改动。" : "保存名称、说明和访问地址，即可纳入应用目录。"}</p></div><button type="button" onClick={onClose} aria-label="关闭" className="rounded-md p-1 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700"><X size={18} /></button></div>

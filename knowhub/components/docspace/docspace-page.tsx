...
import { DocSpaceCard } from "@/knowhub/components/docspace/docspace-card";
import { DocSpaceCreateDialog } from "@/knowhub/components/docspace/docspace-create-dialog";
...
import { listDocspaceItemsApi } from "@/knowhub/features/docspace/api";
...
export function KnowHubDocspacePage() {
  const [keyword, setKeyword] = useState("");
  const [docspaceItems, setDocspaceItems] = useState<DocSpaceRecord[]>([]);
  ...
  async function loadDocspaceItems() {
    setLoading(true);
    setError("");

    try {
      const nextItems = await listDocspaceItemsApi();
      setDocspaceItems(nextItems);
    } catch (loadError) {
  ...
  useEffect(() => {
    void loadDocspaceItems();
  }, []);

  const normalizedKeyword = keyword.trim().toLowerCase();
  const filteredItems = docspaceItems.filter((item) => {
  ...
  return (
    <KnowHubPageShell>
      ...
      <DocSpaceCreateDialog
        ...
        onCreated={() => void loadDocspaceItems()}
      />
    </KnowHubPageShell>
  );
}
